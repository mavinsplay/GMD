...
#     def __init__(self, pos, scale, all, var=1):
#         super().__init__(all)
#         self.original_image = load_image('ShipPortal.png')
#         self.width = self.original_image.get_width()
#         self.height = self.original_image.get_height()
#
#         self.image = pygame.transform.scale(load_image("ShipPortal.png"),
#                                             (int(self.width * scale), int(self.height * scale)))  # удалено subsurafce - ошибка ValueError: subsurface rectangle outside surface area
#         self.mask = pygame.mask.from_surface(self.image)
#         self.rect = self.image.get_rect()
#
#         self.rect.x = pos[0]
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        for i in self.all_sprites:
            if pygame.sprite.collide_mask(i, self) and (type(i) == Stone or type(i) == Spike):
                if type(i) == Spike and self.width * 0.1 < abs(self.rect.x - i.rect.x) < self.width * 0.9:  # убрана проверка на падение в пустоту из-за ошибки
                    exit()  # TODO
