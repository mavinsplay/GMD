...
class GMD: # основной класс объединяющий все модули, реализующий всю игру
    def __init__(self, fps: int, width: int, height: int) -> None:
        ...
        with open('player.txt', 'r+') as file:  # загрузка никнейма игрока для БД рекородов
            data = file.readline()
            if not data:
                file.write('player')
        ...
        self.init_start_buttons() # инициализация всех кнопок
        self.init_icons_buttons()
        self.init_levels_buttons()

    def init_music(self, path: str = 'data/menuLoop.mp3', time: int = -1) -> None: # загрузка музыки
    ...
    def back_button_callback(self, call: str) -> None: # callback для кнопки назад
    ...
    def sql_write(self, score: int, level: int): # метод для записи рекордов в БД
        try:
    ...
    def restart_window(self, level: int = 1, progress: int = 100, func: ... = None): # окно рестарта
    ...
    def set_icon_button(self) -> None: # окно смены скина персонажа
        self.screen.fill((0, 0, 0))
        self.screen.blit(self.background1, (0, 0))
    ...
    def start_level(self, level_nr: int = 1, func: ... = None): # метод запуска уровня из памяти
        if func is None:
...
if __name__ == "__main__":
    gmd = GMD(100, 1500, 800) # создание приложения gmd с парметрами fps, width, height
    gmd.start_window()
